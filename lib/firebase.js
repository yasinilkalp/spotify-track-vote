...
export const addTrack = async (track, user) => {
    ...
    try {
        ...
        let userLimitControl = await userTrackLimitControl(trackRef, user);
        if(!userLimitControl){
            toast.warn('Daha fazla şarkı ekleyemezsiniz.');
            return false;
        }
        ...
        if (tracks.size === 0) {
            ...
            toast.success('Teşekkürler! Şarkıyı listeye eklediniz.');
            return true;
        } else {
            toast.warn('Bu şarkı daha önce eklenmiştir.');
        }
    } catch (e) {
...
const userTrackLimitControl = async (trackRef, user) => {
    try { 
        const checkQuery = query(trackRef, where("user.uid", "==", user.uid));
        let tracks = await getDocs(checkQuery);
...
export const deleteTrack = async (track, user) => {
    try {
        ...
        let tracks = await getDocs(checkQuery);
        if (tracks.size === 0) {
            toast.error('Bu şarkıyı silmek için yetkiniz yok.');
            return false;
        }
...
export const voteTrack = async (track, user) => {
    try {
        ...
        let tracks = await getDocs(checkQuery);
        if (tracks.size === 0) {
            toast.error('Bu şarkıyı güncellemek için yetkiniz yok.');
            return false;
        }
        tracks.forEach(x => {
            var data = x.data();
            ...
            if (data.user.uid == user.uid) {
                toast.warn("Kendi eklediğiniz şarkıya oy veremezsiniz.");
                return false;
            }
